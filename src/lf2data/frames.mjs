...
/**
 * Ticks until this fighter's next damaging hitbox goes live, or `null` if none
 * does inside the horizon. `0` means it is live right now.
 *
 * Follows the frame chain through `next`, which is where the engine goes when
 * the current frame's `wait` runs out.
 */
export function ticksToHit(frames, frameId, waiting = 0, horizon = 12) {
  ...
  for (let hop = 0; hop < 6; hop++) {
    ...
    if (damagingItr(frame).length > 0) return Math.max(0, elapsed);
